...
class Public_model extends CI_Model
{
    ...
    public function setOrder($post){
        ...
        $a_date      = date("Y").'-'.date("m").'-'.'30';
        $akhir_bulan = date("Y-m-t", strtotime($a_date));
        $q = $this->db->query("SELECT MAX(order_id) as order_id, MAX(id) as urut FROM orders where date 
            BETWEEN '$awal_bulan'AND '$akhir_bulan'");
        $format=$this->db->query("SELECT value from value_store where thekey='FormatOrderId'")->row()->value;
        $rr = $q->row_array();
     //   var_dump($rr).'<br>';
        if ($rr['order_id'] == 0 && $rr['urut'] == null) {
            $urut=1;
            $post['order_id'] = $format.$now.'-'.$urut;
        }else{
            $urut=$rr['urut']+1;
            $post['order_id'] = $format.$now.'-'.$urut;
        }
      ...
      //  $i = 0;
       ...
       for ($i=0; $i <$post['total_record'] ; $i++) { 
           $post['products'][$i] = $post['quantity'.$i];
       }
      //  unset($post['id'], $post['quantity']);
       ...
       //var_dump($post);
        ...
        $lastId = $this->db->insert_id();
         for ($a=0; $a < $post['total_record']; $a++) { 
            echo $a.'<br>';
            $getPrice=$this->db->query("SELECT sum(price) as price from products_translations 
            WHERE for_id='".$post['id'.$a]."'")->row()->price;

            if (!$this->db->insert('orders_details', array(
                    'order_id' => $post['order_id'],
                    'product_id' => $post['id'.$a],
                    'quantity' => $post['quantity'.$a],
                    'price' => $getPrice
                ))) {
            log_message('error', print_r($this->db->error(), true));
            }
    ...
    public function registerUser($post){
        $pass=md5($this->input->post('password_register'));
        $key= '@@##@@SUKRON@@##@@';
        $hash=base64_encode($key.$pass.$key);
    ...
    public function checkPublicUserIsValid($post){
        ...
        $this->db->where('password', md5($hash));
    ...
    public function checkLoginAfterRegister($email,$pass){
        $pass=md5($this->input->post('password_register'));
        $key= '@@##@@SUKRON@@##@@';
        $hash=base64_encode($key.$pass.$key);
        $query=$this->db->query("SELECT * FROM users_public where email='$email' AND password='$hash'")->result();
        return $query;
    }
