...
class Loader extends MY_Controller
{
    ...
    /*
     * Load images comming with template in folder /assets/imgs/another folder 
     * Can call from view with template/imgs/another_folder/filename.jpg
     *  PROJECT UPDATE
     */

    public function templateAnotherFolderImage($file, $template = null)
    {
        if ($template == null) {
            $template = $this->template;
        } else {
        ...
        $path2 = VIEWS_DIR . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . scandir('imgs') . DIRECTORY_SEPARATOR .VIEWS_DIR. $file;
        ...
        $image_mime2 = null;
        if (function_exists('mime_content_type')) {
            $image_mime2 = mime_content_type($path2);
        } elseif (function_exists('finfo_open')) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $image_mime2 = finfo_file($finfo, $path2);
            finfo_close($finfo2);
        }
        if ($image_mime2 !== null) {
            header('Content-Type: ' . $image_mime2 . '  charset: UTF-8');
        }   
        echo $img2;     
    }    
